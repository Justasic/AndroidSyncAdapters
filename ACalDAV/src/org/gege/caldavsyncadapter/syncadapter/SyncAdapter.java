...
public class SyncAdapter extends AbstractThreadedSyncAdapter
{
	...
	public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult)
	{
		...
		try
		{
			...
			{
				lcUsername = mAccountManager.getUserData(account, AuthenticatorActivity.USER_DATA_USERNAME);
			}

			CaldavFacade facade = new CaldavFacade(lcUsername, mAccountManager.getPassword(account), url, trust);
				...
				else
				{
					...
					if (mCountProviderFailed >= mCountProviderFailedMax)
					{
						...
						NotificationsHelper.signalSyncErrors(
						    this.getContext(), getContext().getString(R.string.error_caldav_sync_provider_failed),
		...
		catch (SSLException e)
		{
			...
			{
				NotificationsHelper.signalSyncErrors(this.getContext(), getContext().getString(R.string.error_caldav_sync_ssl), e.getMessage());
			}
		}
		...
		catch (final ParseException e)
		{
			...
			NotificationsHelper.signalSyncErrors(this.getContext(), getContext().getString(R.string.error_caldav_sync_parse), e.getMessage());
		}
		catch (Exception e)
		{
			Log.e(TAG, "Updating calendar exception " + e.getClass().getName(), e);
			syncResult.stats.numParseExceptions++;
			NotificationsHelper.signalSyncErrors(this.getContext(), getContext().getString(R.string.error_caldav_sync_general), e.getMessage());
		}
	}
	 ...
	 */
	private void synchroniseEvents(DavCalendar androidCalendar, DavCalendar serverCalendar, SyncStats stats, ArrayList<Uri> notifyList)
	    throws ClientProtocolException, URISyntaxException, IOException, ParserConfigurationException, SAXException, RemoteException, CaldavProtocolException,
	           ParserException
	{
		...
		for (CalendarEvent calendarEvent : serverCalendar.getCalendarEvents())
		{
			...
			catch (ParserException ex)
			{
				...
				NotificationsHelper.signalSyncErrors(this.getContext(), getContext().getString(R.string.error_caldav_sync_parse), ex.getMessage());
			}
			catch (CaldavProtocolException ex)
			{
				Log.e(TAG, "Caldav exception", ex);
				stats.numParseExceptions++;
				NotificationsHelper.signalSyncErrors(this.getContext(), getContext().getString(R.string.error_caldav_sync_caldav), ex.getMessage());
			}
		}
		...
		try
		{
			...
			while (curEvent.moveToNext())
			{
				try
				{
					...
					Log.w(
					    "EVENTS",
					    "EvtId:" + lcEventID + " Cancelled:" + Cancelled + " OriginalSyncId:" + originalSyncId + " exdate:" + exdate + " - rrule:" + rrule);
	...
	private int checkDirtyAndroidEvents(
	    ContentProviderClient provider, Account account, Uri calendarUri, CaldavFacade facade, URI caldavCalendarUri, SyncStats stats,
				...
				else
				{
					...
					if (uid != null)
					{
						...
						if (facade.updateEvent(URI.create(SyncID), androidEvent.getIcsEvent().toString(), androidEvent.getETag()))
						{
							...
							int RowCount
							    = provider.update(asSyncAdapter(androidEvent.getUri(), account.name, account.type), androidEvent.ContentValues, null, null);
